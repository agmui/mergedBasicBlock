...
int FunctionComparator_cp::cmpNumbers(uint64_t L, uint64_t R) const {
    if (L < R)
        return -1;
    if (L > R)
        return 1;
...
int FunctionComparator_cp::cmpAligns(Align L, Align R) const {
    if (L.value() < R.value())
        return -1;
    if (L.value() > R.value())
        return 1;
...
int FunctionComparator_cp::cmpOrderings(AtomicOrdering L, AtomicOrdering R) const {
    if ((int)L < (int)R)
        return -1;
    if ((int)L > (int)R)
        return 1;
...
int FunctionComparator_cp::cmpAPInts(const APInt &L, const APInt &R) const {
...
int FunctionComparator_cp::cmpConstantRanges(const ConstantRange &L,
...
int FunctionComparator_cp::cmpAPFloats(const APFloat &L, const APFloat &R) const {
...
int FunctionComparator_cp::cmpMem(StringRef L, StringRef R) const {
...
int FunctionComparator_cp::cmpAttrs(const AttributeList L,
...
int FunctionComparator_cp::cmpMetadata(const Metadata *L,
...
int FunctionComparator_cp::cmpMDNode(const MDNode *L, const MDNode *R) const {
    if (L == R)
        return 0;
    if (!L)
        return -1;
...
int FunctionComparator_cp::cmpInstMetadata(Instruction const *L,
...
int FunctionComparator_cp::cmpOperandBundlesSchema(const CallBase &LCS,
...
int FunctionComparator_cp::cmpConstants(const Constant *L,
...
int FunctionComparator_cp::cmpGlobalValues(GlobalValue *L, GlobalValue *R) const {
...
int FunctionComparator_cp::cmpTypes(Type *TyL, Type *TyR) const {
    PointerType *PTyL = dyn_cast<PointerType>(TyL);
    PointerType *PTyR = dyn_cast<PointerType>(TyR);
...
int FunctionComparator_cp::cmpOperations(const Instruction *L,
                                      const Instruction *R,
...
// Read method declaration comments for more details.
int FunctionComparator_cp::cmpGEPs(const GEPOperator *GEPL,
...
int FunctionComparator_cp::cmpInlineAsm(const InlineAsm *L,
...
int FunctionComparator_cp::cmpValues(const Value *L, const Value *R) const {
...
int FunctionComparator_cp::cmpBasicBlocks(const BasicBlock *BBL,
...
int FunctionComparator_cp::compareSignature() const {
...
int FunctionComparator_cp::compare() {
    beginCompare();
